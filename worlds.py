"""Deterministic external worlds for the Auxein v0.5.0 laboratory."""
...
@dataclass(slots=True)
class WeightedPresentationCycle:
    presentations: list[list[list[object]]]

    def sample(self, step: int) -> Sample:
        index = step % len(self.presentations)
        value = copy.deepcopy(self.presentations[index])
        return Sample(value, {"presentation": index, "size": len(value), "weighted": True})
...
def build_world(spec: Mapping[str, object], *, seed: int) -> World:
    ...
    if name == "point-cycle":
        return PointCycle(
            _vectors(spec.get("points"), "world.points"),
    ...
    if name == "weighted-presentation-cycle":
        raw = spec.get("presentations")
        if not isinstance(raw, list) or not raw:
            raise ValueError("world.presentations must be a nonempty list")
        presentations: list[list[list[object]]] = []
        for pi, presentation in enumerate(raw):
            if not isinstance(presentation, list) or not presentation:
                raise ValueError(f"world.presentations[{pi}] must be nonempty")
            atoms: list[list[object]] = []
            for ai, atom in enumerate(presentation):
                if not isinstance(atom, list) or len(atom) != 3 or not isinstance(atom[1], list):
                    raise ValueError(f"world.presentations[{pi}][{ai}] must be [W,C,V]")
                atoms.append([float(atom[0]), [float(x) for x in atom[1]], float(atom[2])])
            presentations.append(atoms)
        return WeightedPresentationCycle(presentations)
